...
// Convert any value to a DB-safe numeric (null for blank/NaN)
...
const toDb = q => ({
  ...
  misa_items: q.misaItems??null, misa_total: num(q.misaTotal),
  misa_total_words: q.misaTotalWords??null,
  bookkeeping_rate:      num(q.bookkeepingRate),
  tax_fee:               0,
  audit_hours:           0,
  audit_rate:            0,
  ...
  recruitment_fee: num(q.recruitmentFee), handbook_fee: num(q.handbookFee),
  hr_retainer: num(q.hrRetainer), placements: num(q.placements) ?? 1,
  hr_scope: q.hrScope??null,
  hr_services: q.hrServices??[],
  hr_total: num(q.hrTotal) ?? 0,
